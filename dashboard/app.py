...
def get_health():
    try:
        print(f"Attempting to connect to: {API_BASE}/health") 
...
def score_transaction(payload: dict):
    try:
        r = requests.post(f"{API_BASE}/score", json=payload, timeout=60)
        return r.json()
    except Exception as e:
        return {"error": str(e)}
...
with st.sidebar:
    ...
    # API health and error revealing
    health, error_msg = get_health()
    if health:
        ...
        st.caption(f"**Target URL:** `{API_BASE}/health`")
        st.code(f"Error Details:\n{error_msg}", language="bash")
        health = {}
