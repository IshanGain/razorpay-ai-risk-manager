...
from mlops.drift import get_drift_summary
...
with st.sidebar:
    ...
    if health:
        ...
        rzp_ok = health.get("razorpay_connected", False)
        st.caption(f"Razorpay: {'Connected' if rzp_ok else 'Disconnected'}")
...
with tab1:
    ...
    avg_score = stats.get("avg_fraud_score", 0.0)

    c1, c2, c3, c4, c5 = st.columns(5)
    c1.metric("Total Decisions", f"{total:,}")
    c2.metric("Approved",    f"{approve:,}", f"{approve/max(total,1):.1%}")
    c3.metric("Step-Up 2FA", f"{step_up:,}", f"{step_up/max(total,1):.1%}")
    c4.metric("Declined",    f"{decline:,}", f"{decline/max(total,1):.1%}")
    c5.metric("Avg P(Fraud)", f"{avg_score:.4f}")
    ...
    if history:
        ...
        for rec in history:
            ...
            rows.append({
                ...
                "P(Fraud)": f"{rec.get('p_fraud', 0):.4f}",
                "Amount":   f"₹{rec.get('amount', 0):,.0f}",
    ...
    with col_result:
        ...
        if score_btn:
            ...
            if "error" not in result:
                ...
                color = {"APPROVE": "green", "STEP_UP_2FA": "orange",
                         "DECLINE": "red"}.get(decision, "blue")
                emoji = {"APPROVE": "✅", "STEP_UP_2FA": "⚡",
    ...
    if history:
        ...
        for rec in history:
            ...
            rows.append({
                ...
                "Amount":     f"₹{rec.get('amount', 0):,.0f}",
                "Merchant":   rec.get("merchant_id", "N/A"),
...
with tab6:
    ...
    if run_drift:
        ...
        if status not in ["INSUFFICIENT_DATA", "UNKNOWN"]:
            ...
            c3.metric("Recent Scores",    summary.get("recent_count", 0))
            c4.metric("Reference Scores", summary.get("reference_count", 0))
            ...
            st.info(
                ...
                f"Current: {summary.get('recent_count', 0)} recent scores. "
                f"Need at least 10."
            )
    else:
...
**KL Divergence** is a complementary signal measuring information loss
